package org.alumno.carlos.carlos_primer_app_spring_mvc.utils;
...
public class Ts {
	...
	public static Date parseDate(String string, String format) {
		...
		try {
			if (string != null)
				if (!string.equals(""))
					date = new SimpleDateFormat(format).parse(string.substring(0, format.length() - 1));
		} catch (ParseException e) {
		}
		return date;
	}
	...
	public static String formatIso(Date date) {
		if (date == null)
			return null;// evitar error fecha nula
	...
	public static Date parseIsoMin(String string) {
		...
		try {
			if (string != null)
				if (!string.equals(""))
					date = new SimpleDateFormat("yyyy-MM-dd'T'00:00:00").parse(string);
		} catch (ParseException e) {
		}
		return date;
	}
	...
	public static int diffMonth(Calendar from, Calendar to) {
		int diff = Math.abs(from.get(Calendar.YEAR) - to.get(Calendar.YEAR)) * 12
